...
export default function RightPanel({ user, arr,info }) {
    ...
    return (
        <div className="flex flex-col items-start">
            ...
            <hr className="ml-auto mt-8 mx-12 right-0 w-[65%]  border-1 border-solid rounded-xl  " />
            <div className=" w-full font-ubuntu">
            
                
            {/* Banner Container */}
                <div className=' ml-auto  mx-12 right-0 w-[65%] border-gray-100  p-4 rounded-xl mt-2 mb-2'>
                    <div className="flex align-middle ">
                        <div id="results" className = {`flex justify-center border-2 mx-0 ${currentTab==='results'? 'text-custom-red border-b-custom-red':''} cursor-pointer  border-x-transparent border-t-transparent px-5 py-3`} onClick={handleClick}><h1 className="font-medium text-lg">Results</h1></div>
                        <div id="experiences" className={`flex mx-0 justify-center border-2  ${currentTab==='experiences'? 'text-custom-red border-b-custom-red':''} cursor-pointer border-x-transparent border-t-transparent px-5 py-3`} onClick={handleClick}><h1 className="font-medium text-lg">Experiences</h1></div>
                       

                    </div>
                </div>
            </div>
            {currentTab === 'experiences' ? <ExperienceSection user={user} array_exp={arr} />:<Results />}
